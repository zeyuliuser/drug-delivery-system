	...
	<div class="app-dashboard shrink-medium">
		...
		<div class="app-dashboard-body off-canvas-wrapper">
			<div id="app-dashboard-sidebar"
				...
				<div>
					<ul id="collapsing-tabs" class="menu vertical tabs">
						...
						<li class="tab-title  is-active"><a href="${path}com/servlet/ZcompGetAllDrugServlet"> <i
			...
			<div
				...
				<form action="${path}com/servlet/ZcompSearchDrugByClass" method="post">
				    <jsp:include page="drugsearchlabel.jsp"></jsp:include>
				    <button class="button" type="submit">查找</button>
                    </form>                
					<c:forEach items="${list}" var="drug">
					<form action="${path}com/servlet/DeleteDrugServlet" method="post">
						<div class="card-info primary">
							<div class="card-info-content">
							<input type="text" name="did" value="${drug.did}" style="display:none">
								<h3 class="lead">${drug.dname}</h3>
								<p>${drug.ddesc}</p>
								<p>价格:${drug.price}</p>
								<button type="submit" class="button warning">删除</button>
							</div>
						</div>
					</form>
					</c:forEach>
