			...
			<div id="app-dashboard-sidebar"
				...
				<div>
					<ul id="collapsing-tabs" class="menu vertical tabs">
						...
						<li class="tab-title is-active"><a href="${path}com/servlet/ShowCartServlet"> <i
			...
			<div
				...
				<div class="tabs-panel content is-active">
					 <c:forEach items="${list}" var="cart">
						...
						<form action="${path}com/servlet/DeleteCart" method="post">
							<div class="card-info-content">		
							    <input type="text" name="cartid" value="${cart.id}" style="display:none">					
								<h3>药品名称：${cart.dname}</h3>								
								<p>单价:${cart.price}</p>
								<p>数量:${cart.num}</p>
								<p>总价价:${cart.amount}</p>
								<button type="submit" class="button warning">删除商品</button>
							</div>
						</form>
						</div>
					</c:forEach>
					<a href="${path}buginfo.jsp"><button class="button">提交订单</button></a>
				</div>
			</div>
		</div>
	</div>
